class TaxableIncome
  ...
  def initialize(tax_brackets)
    @tax_brackets = tax_brackets.sort_by {|hsh| hsh[:lowest_amount]}.reverse!
  end
  ...
  def calculate_total_amount(taxed_income)
    ...
    tax_brackets.each do |bracket|
      amount_over_lowest_amount = taxed_income - bracket[:lowest_amount]

      if amount_over_lowest_amount > 0
        taxed_income     -= amount_over_lowest_amount
        total_tax_amount += amount_over_lowest_amount * bracket[:percentage]
      else
        total_tax_amount += 0
      end
  ...
  #    if tax bracket 1 has lowest_amount 10,000
  #    and tax bracket 2 has lowest_amount 20,000
